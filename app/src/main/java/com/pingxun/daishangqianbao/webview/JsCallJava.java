...
public class JsCallJava {
    ...
    public JsCallJava(Class injectedCls) {
        try {
            ...
            StringBuilder sb = new StringBuilder(
                    "javascript:(function(b){console.log(\"HostApp initialization begin\");var a={queue:[],callback:function(){var d=Array.prototype.slice.call(arguments,0);var c=d.shift();var e=d.shift();this.queue[c].apply(this,d);if(!e){delete this.queue[c]}}};");
    ...
    private String getReturn(String reqJson, int stateCode, Object result) {
        ...
        if (result == null) {
            ...
            insertRes = "\"" + result + "\"";
        } else if (!(result instanceof Integer) && !(result instanceof Long) && !(result instanceof Boolean) && !(result instanceof Float)
                && !(result instanceof Double) && !(result instanceof JSONObject)) {    // 非数字或者非字符串的构造对象类型都要序列化后再拼接
            try {
                insertRes = new Gson().toJson(result);
            } catch (Exception e) {
                stateCode = 500;
